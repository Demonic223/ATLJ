...
begin
  ...
  MySQLComp.SetQuery('INSERT INTO accounts (username, password, password_hash, last_token_creation_time'+
    ...
    ') VALUES ('+QuotedStr(Username)+','+QuotedStr(Password)+','+ QuotedStr(TFunctions.StringToMd5(Password)) +',1,2,0,0,0,0,0,' + QuotedStr('::0')+ ',1)');
  MySQLComp.Run(False);
...
var
  ...
  Player: TPlayer;
  LNow: TDateTime; // Variable for a consistent time check
begin
  ZeroMemory(@Player, SizeOf(Player));
  if (Player.LoadAccountSQL(string(Packet.Username))) then
  begin
    // Store the current time once to ensure consistency during the check.
    LNow := Now;

    // --- OPTIONAL BUT RECOMMENDED FOR DEBUGGING ---
    // This will print the exact times being compared to your server log.
    Logger.Write('Token Check Time - Now: ' + DateTimeToStr(LNow) +
      ' | TokenCreation: ' + DateTimeToStr(Player.Account.Header.Token.CreationTime) +
      ' | SecondsBetween: ' + IntToStr(SecondsBetween(LNow, Player.Account.Header.Token.CreationTime)), TLogType.ConnectionsTraffic);
    // --- END DEBUGGING ---

    // The corrected check uses Abs() to get the absolute difference in seconds.
    if (Player.Account.Header.Token.Token = Packet.Token) and
      (Abs(SecondsBetween(LNow, Player.Account.Header.Token.CreationTime)) < 300) then
    begin
