...
type
  ...
  public
    ...
    procedure SendMessageToPainel(StrTo: String; Icon: WORD; Action: WORD);
  end;
{$OLDTYPELAYOUT OFF}
implementation
...
begin
  ...
  try
    ...
    var loadedInt: Integer;
    loadedInt := PlayerSQLComp.Query.FieldByName('last_token_creation_time').AsInteger;
    Logger.Write('[DIAGNOSTIC 4] LoadAccountSql - Integer value read from DB: ' + IntToStr(loadedInt), TLogType.Warnings);
    ...
    Self.Account.Header.Token.CreationTime :=
     UnixToDateTime(PlayerSQLComp.Query.FieldByName('last_token_creation_time').AsInteger);
   ...
   // else
      Self.Account.Header.PremiumTime :=
        UnixToDateTime(PlayerSQLComp.Query.FieldByName('premium_time').AsInteger);
...
begin
  ...
  try
  Logger.Write('[DIAGNOSTIC 2] SaveAccountToken - Preparing to save TDateTime value: ' + DateTimeToStr(Self.Account.Header.Token.CreationTime), TlogType.Warnings);
  Logger.Write('[DIAGNOSTIC 3] SaveAccountToken - Preparing to save Unix timestamp: ' + IntToStr(DateTimeToUnix(Self.Account.Header.Token.CreationTime)), TlogType.Warnings);
        SQLComp.SetQuery('UPDATE accounts SET last_token = ' + QuotedStr(String(Self.Account.Header.Token.Token)) + ',' +
      ' last_token_creation_time = ' + IntToStr(DateTimeToUnix(Self.Account.Header.Token.CreationTime)) +
...
begin
  ...
  try
    ...
    ', ' +
      'premium_time='+IntToStr(DateTimeToUnix(Self.Account.Header.PremiumTime))+
...
begin
  ...
  begin
    ...
    begin
      Packet.CharactersData[i].DeleteTime := DateTimeToUnix
...
{$ENDREGION}
{$REGION 'Classes'}
class procedure TPlayer.ForEach(proc: TProc<PPlayer, TParallel.TLoopState>; Server: BYTE);
begin
  TParallel.For(1, Servers[Server].InstantiatedPlayers + 1,
...
procedure TPlayer.SendMessageToPainel(StrTo: String; Icon: WORD; Action: WORD);
var
  Packet: TMessageToPainelPacket;
begin
  ZeroMemory(@Packet, sizeof(TMessageToPainelPacket));
  Packet.Header.Size := sizeof(TMessageToPainelPacket);
  Packet.Header.Code := $3232;
  Packet.Header.Index := Self.Base.ClientID;
  Packet.Type1 := Icon;
  Packet.Type2 := Action;
  AnsiStrings.StrPCopy(Packet.Msg, StrTo);
  Self.SendPacket(Packet, Packet.Header.Size);
end;
end.
